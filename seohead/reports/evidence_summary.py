...
def rows(summary: dict[str, Any]) -> list[list[str]]:
    ...
    if not isinstance(contract, dict):
        return []
    output = [["Run", "Saved scan identity", str(contract.get("scan_identity_state", "unavailable")), str(contract.get("scan_uuid") or contract.get("scan_identity_reason") or "not retained")]]
