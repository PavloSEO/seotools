...
def put(con: Any, occurrence: dict[str, Any]) -> None:
    required = {
        ...
        "representation", "carrier", "raw_value", "resolved_value", "depth", "outcome", "reason",
    }
    ...
    if len(occurrence["occurrence_key"]) > 512 or any(len(occurrence[key]) > 8192 for key in ("raw_value", "resolved_value", "reason")):
    ...
    con.execute(
        ...
        (
            ...
            occurrence["source_url_id"], occurrence["source_document_id"], occurrence["representation"],
            occurrence["carrier"], occurrence["raw_value"], occurrence["resolved_value"], target_id,
            occurrence["depth"], occurrence["outcome"], occurrence["reason"],
        ),
    )


def _relation_items(html: str | None, source_url: str, headers: dict[str, Any] | None) -> list[tuple[str, str, str, str]]:
    ...
    if isinstance(html, str):
        ...
        if meta is not None:
            ...
            if match:
                ...
                items.append(("refresh", "meta[http-equiv=refresh]", raw, urljoin(source_url, target)))
        ...
        if isinstance(raw_header, str):
            for ordinal, part in enumerate(raw_header.split(",")):
                ...
                if match:
                    ...
                    relation = (
                        "canonical" if "canonical" in rels else "alternate" if "alternate" in rels
                        else "next" if "next" in rels else "prev" if "prev" in rels else "http_link"
                    )
...
def store_document_relations(
    con: Any, *, source_url_id: int, source_document_id: int | None, representation: str,
...
def store_seeds(con: Any, entries: list[dict[str, Any]]) -> None:
    for ordinal, item in enumerate(entries):
        ...
        put(con, {"occurrence_key": f"seed:{ordinal}:{raw}", "source_kind": "seed", "relation": "seed", "source_url_id": None, "source_document_id": None, "representation": "unmeasured", "carrier": str(item.get("source") or "seed"), "raw_value": raw, "resolved_value": resolved, "depth": item.get("depth"), "outcome": outcome, "reason": reason})
...
def read(con: Any, *, limit: int = 1000, offset: int = 0) -> dict[str, Any]:
    ...
    return {"schema_version": VERSION, "state": "complete", "items": rows[:limit], "truncated": len(rows) > limit}
