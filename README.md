...
72 callable tools · 161 checks · 30 workflow skills · 56 scenarios · 2 400+ offline tests · CLI · local MCP · Docker
...
| A tool-calling AI agent | `seohead mcp` | 67 shared `seo_*` handlers plus five `sf_*` crawl-workflow tools over local stdio |
...
### 67 core CLI commands and MCP tools
...
| Project workspaces | 3 | portable project creation, opening, and truthful pending status |
...
<details>
<summary><b>All 67 commands, by what they answer</b></summary>
...
The audit layer does not care where the crawl came from. Three sources feed the same 161-check
...
The analyzer has a registry of **161 checks** across metadata, indexability, canonicals, redirects,
...
The server exposes **67 `seo_*` tools plus five `sf_*` tools**. The 67 core tools share the tested
...
- `site-audit` is a bounded sitemap-based evidence pass, not an exhaustive run of all 67 core
