...
69 callable tools · 157 checks · 29 workflow skills · 56 scenarios · 2 400+ offline tests · CLI · local MCP · Docker
...
consumed by SEOHEAD's 157-check analyzer, and remains the stronger choice for web-scale crawls.
SEOHEAD also ships its own bounded native crawler (`crawl-site`) for when no SF licence is
installed: it fetches a site directly and feeds the same 157-check registry, but it is not
...
|---|---|---|
| Crawl collection | Screaming Frog, or SEOHEAD's own bounded `crawl-site` when no SF licence is installed | Discover site URLs and produce evidence for the 157-check registry — SF for web-scale crawls, `crawl-site` for a licence-free bounded pass |
| Evidence processing | SEOHEAD Tools | Analyze that evidence against a 157-check registry, run targeted live and infrastructure tools, preserve uncertainty, and build structured artifacts |
...
|---|---|---|
| **A site, and no crawl yet** | `seohead crawl-site --url https://example.com --out-dir ./run` | Crawls the site with this toolkit's own crawler — no Screaming Frog, no licence — audits the result through the same 157-check registry, and writes `audit.json` beside a prioritized `tasks.md` backlog |
| **Existing Screaming Frog exports** | `seohead sf run --exports-dir ./exports --out ./report --tasks` | Evaluates crawl evidence you already have against the 157-check registry and builds an audit plus a prioritized backlog |
...
| A tool-calling AI agent | `seohead mcp` | 64 shared `seo_*` handlers plus five `sf_*` crawl-workflow tools over local stdio |
...
### 64 core CLI commands and MCP tools
...
| Audit orchestration and reporting | 7 | bounded sitemap-based site evidence, the crawler's own configuration surface, XLSX/DOCX/CSV/Markdown/JSON output, a no-network multi-site facts export, offline SQLite reanalysis, a cross-segment counterpart diff that answers what one language version has and another does not, and the join that carries an operator's own external rows alongside crawled evidence without letting them decide anything |
...
<details>
<summary><b>All 64 commands, by what they answer</b></summary>

**Crawl a site yourself** — `crawl-site` · `crawl-enrich` · `crawl-describe-settings` ·
`compare-crawls` · `log-scan`
...
`scan-prune` · `scan-body-diff` · `scan-reanalyze` · `segment-diff`
...
The audit layer does not care where the crawl came from. Three sources feed the same 157-check
...
The analyzer has a registry of **157 checks** across metadata, indexability, canonicals, redirects,
...
The server exposes **64 `seo_*` tools plus five `sf_*` tools**. The 64 core tools share the tested
